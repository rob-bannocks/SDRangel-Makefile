...
BUILDROOT=/opt/build
USER=robert
all: setup APT CM265cc LibDAB MBElib SerialDV DSDcc FreeDV SGP4 LibSigMF RTL-SDR SoapySDR-RTL-SDR SDRangel
#
...
APT: aptdec
	cd $(BUILDROOT)/aptdec && git checkout libaptdec && git submodule update --init --recursive && mkdir -p build && cd build && cmake -Wno-dev -DCMAKE_INSTALL_PREFIX=/opt/install/aptdec .. && make -j $(nproc) install 
...
CM256cc: cm256cc
	cd $(BUILDROOT) && cd cm256cc && git reset --hard c0e92b92aca3d1d36c990b642b937c64d363c559 && mkdir -p build && cd build && cmake -Wno-dev -DCMAKE_INSTALL_PREFIX=/opt/install/cm256cc .. && make -j $(nproc)
...
LibDAB: dab-cmdline
	cd $(BUILDROOT) && cd dab-cmdline/library && git checkout msvc && mkdir -p build && cd build && cmake -Wno-dev -DCMAKE_INSTALL_PREFIX=/opt/install/libdab .. && make -j $(nproc) install

dab-cmdline:
	cd $(BUILDROOT) && git clone https://github.com/srcejon/dab-cmdline

MBElib: mbelib
	cd $(BUILDROOT) && cd mbelib && git reset --hard 9a04ed5c78176a9965f3d43f7aa1b1f5330e771f && mkdir -p build && cd build && cmake -Wno-dev -DCMAKE_INSTALL_PREFIX=/opt/install/mbelib .. && make -j $(nproc) install

mbelib:
	cd $(BUILDROOT) && git clone https://github.com/szechyjs/mbelib.git

SerialDV: serialDV
	cd $(BUILDROOT) && cd serialDV && git reset --hard "v1.1.4" && mkdir -p build && cd build && cmake -Wno-dev -DCMAKE_INSTALL_PREFIX=/opt/install/serialdv .. && make -j $(nproc) install

serialDV:
	cd $(BUILDROOT) && git clone https://github.com/f4exb/serialDV.git

DSDcc: dsdcc
	cd $(BUILDROOT) && cd dsdcc && git reset --hard "v1.9.3" && mkdir -p build && cd build && cmake -Wno-dev -DCMAKE_INSTALL_PREFIX=/opt/install/dsdcc -DUSE_MBELIB=ON -DLIBMBE_INCLUDE_DIR=/opt/install/mbelib/include -DLIBMBE_LIBRARY=/opt/install/mbelib/lib/libmbe.so -DLIBSERIALDV_INCLUDE_DIR=/opt/install/serialdv/include/serialdv -DLIBSERIALDV_LIBRARY=/opt/install/serialdv/lib/libserialdv.so .. && make -j $(nproc) install
...
FreeDV Codec2: codec2
	...
	cd $(BUILDROOT) && cd codec2 && git reset --hard 76a20416d715ee06f8b36a9953506876689a3bd2 && mkdir -p build_linux && cd build_linux && cmake -Wno-dev -DCMAKE_INSTALL_PREFIX=/opt/install/codec2 .. && make -j $(nproc) install

codec2:
	cd $(BUILDROOT) && git clone https://github.com/drowe67/codec2.git

SGP4: sgp4
	cd $(BUILDROOT) && cd sgp4 && mkdir -p build && cd build && cmake -Wno-dev -DCMAKE_INSTALL_PREFIX=/opt/install/sgp4 .. && make -j $(nproc) install

sgp4:
	cd $(BUILDROOT) && git clone https://github.com/dnwrnr/sgp4.git

LibSigMF: libsigmf
	cd $(BUILDROOT) && cd libsigmf && git checkout "new-namespaces" && mkdir -p build && cd build && cmake -Wno-dev -DCMAKE_INSTALL_PREFIX=/opt/install/libsigmf ..  && make -j $(nproc) install

libsigmf:
	cd $(BUILDROOT) && git clone https://github.com/f4exb/libsigmf.git

RTL-SDR: librtlsdr
	cd $(BUILDROOT) && cd librtlsdr && git reset --hard be1d1206bfb6e6c41f7d91b20b77e20f929fa6a7 && mkdir -p build && cd build && cmake -Wno-dev -DDETACH_KERNEL_DRIVER=ON -DCMAKE_INSTALL_PREFIX=/opt/install/librtlsdr .. && make -j $(nproc) install
...
soapy-SDR: SoapySDR-RTL-SDR SoapySDR
	 cd $(BUILDROOT) && cd SoapySDR && git reset --hard "soapy-sdr-0.7.1" && mkdir -p build && cd build && cmake -DCMAKE_INSTALL_PREFIX=/opt/install/SoapySDR .. && make -j $(nproc) install
